...
namespace TinySTL{
	...
	public:
		...
		// 在ReverseIterator.h定义
		typedef reverse_iterator_t<T*>				reverse_iterator;
		typedef reverse_iterator_t<const T*>		const_reverse_iterator;
		
		typedef iterator							pointer; // typedef iterator
		
		typedef T&									reference; // 引用
		typedef const T&							const_reference; //常量引用
		typedef size_t								size_type; //数量类型
		typedef ptrdiff_t							difference_type; //保存迭代器距离

	public:
		//构造，复制，析构相关函数
		vector()
			:start_(0), finish_(0), endOfStorage_(0){}
		explicit vector(const size_type n); //禁止size_type转换为vector
		...
		//迭代器相关
		iterator begin(){ return (start_); } // return T*
