...
namespace TinySTL{
	namespace VectorTest{
		...
		void testCase1(){
			...
			stdVec<std::string> v1(10, "chok");
			tsVec<std::string> v2(10, "chok");
			assert(TinySTL::Test::container_equal(v1, v2)); //测试标准vector和自定义vector的迭代器指向内容相同

			stdVec<std::string> v3(10);
			tsVec<std::string> v4(10);
			assert(TinySTL::Test::container_equal(v3, v4)); //测试标准vector和自定义vector的迭代器指向内容相同

			std::array<std::string, 3> arr = { "abc", "def", "ghi" }; 
			stdVec<std::string> v5(std::begin(arr), std::end(arr)); //使用标准array构造
